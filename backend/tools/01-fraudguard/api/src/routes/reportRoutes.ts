...
router.post("/export/pdf", async (req: Request, res: Response) => {
  try {
    const { start_date, end_date, risk_levels, include_details, title } =
      req.body;
    ...
    doc.on("end", () => {
      ...
      res.setHeader(
        "Content-Disposition",
        `attachment; filename="fraudguard-report-${Date.now()}.pdf"`
      );
      res.send(pdfBuffer);
    });
    ...
    doc
      .fontSize(24)
      .text(title || "FraudGuard Analysis Report", { align: "center" });
    doc.moveDown();
    doc
      .fontSize(12)
    ...
    if (include_details) {
      ...
      filteredTransactions.slice(0, 50).forEach((t, i) => {
        ...
        doc.text(
          `   Risk Score: ${score?.score || "N/A"} (${
            score?.risk_level || "N/A"
          })`
        );
...
router.post("/export/csv", async (req: Request, res: Response) => {
  try {
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename="fraudguard-report-${Date.now()}.csv"`
    );
    res.send(csv);
  } catch (error) {
