...
const NeuralLinkInterface: React.FC = () => {
  ...
  const handleFileUpload = (file: File) => {
    ...
    reader.onload = (e) => {
      ...
      handleSend(
        `I have uploaded a file: ${file.name}. Please analyze it in the workspace.`
      );
    };
          ...
          callbacks: {
            onopen: () => {
              ...
              scriptProcessor.onaudioprocess = (e) => {
                ...
                for (let i = 0; i < binary.length; i++)
                  bStr += String.fromCharCode(binary[i]);
            ...
            onmessage: async (message: LiveServerMessage) => {
              ...
              if (base64) {
                ...
                const buffer = outputCtx.createBuffer(
                  1,
                  dataInt16.length,
                  24000
                );
    ...
    <div className="matrix-bg text-gray-300 h-screen flex flex-col overflow-hidden relative selection:bg-green-500/30 selection:text-white font-mono">
      ...
      <div
        className={`flex flex-col h-full transition-opacity duration-300 ${
          isNavDrawerOpen ? "opacity-0 pointer-events-none" : "opacity-100"
        }`}
      >
        <Header
        ...
        <div className="flex-grow flex relative overflow-hidden z-10">
          {(isLeftPanelOpen || isRightPanelOpen) && (
            <div
              className="absolute inset-0 bg-black/40 backdrop-blur-[2px] z-[55] transition-opacity animate-in fade-in duration-300"
              onClick={() => {
